...
namespace Amuse.UI.UserControls
{
    ...
    public partial class FilePickerTextBox : UserControl
    {
        ...
        {
            var filepath = await _dialogService.OpenFileDialogAsync(Title, InitialDirectory, Filter, DefaultExt);
        ...
        {
            var filepath = await _dialogService.OpenFolderDialogAsync(Title, InitialDirectory);
            if (!string.IsNullOrEmpty(filepath))
                FileName = filepath;
        }
